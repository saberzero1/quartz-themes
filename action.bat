...
set "GITHUB_URL_BASE=https://raw.githubusercontent.com/saberzero1/quartz-themes/refs/heads/master/"
...
set "CSS_EXTRAS_INDEX_URL=%GITHUB_URL_BASE%%GITHUB_EXTRAS_DIR%_index.scss"
...
echo Creating theme directory structure...
mkdir "%TARGET_THEME_DIR%\extras" > nul
if errorlevel 1 (
    >&2 echo "[ERROR] Failed to create directory '%TARGET_THEME_DIR%\extras'. Check permissions."
    exit /b 1
)
echo.
...
curl -s -S -o "%TARGET_THEME_DIR%\extras\_index.scss" "%CSS_EXTRAS_INDEX_URL%" || ( >&2 echo "[ERROR] Failed to download extras/_index.scss" & exit /b 1 )
...
if errorlevel 1 (
    ...
    echo %BASE_IMPORT_LINE% >> "%TEMP_SCSS_FILE%"
    echo %IMPORT_LINE% >> "%TEMP_SCSS_FILE%"
    echo. >> "%TEMP_SCSS_FILE%"
    echo // put your custom CSS here^^! >> "%TEMP_SCSS_FILE%"
