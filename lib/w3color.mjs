...
(function() {
  ...
  function roundDecimals(c) {
    ...
    c.sat = Number(c.sat.toFixed(3));
    c.lightness = Number(c.lightness.toFixed(3));
    c.whiteness = Number(c.whiteness.toFixed(3));
    c.blackness = Number(c.blackness.toFixed(3));
    c.cyan = Number(c.cyan.toFixed(3));
    c.magenta = Number(c.magenta.toFixed(3));
    c.yellow = Number(c.yellow.toFixed(3));
    c.black = Number(c.black.toFixed(3));
    c.ncol = c.ncol.substr(0, 1) + Math.round(Number(c.ncol.substr(1)));
    c.opacity = Number(c.opacity.toFixed(3));
    return c;
  }
  ...
  function ncsToRgb(ncs) {
    let black, chroma, bc, percent, black1, chroma1, /*red1, */factor1, blue1, red1, red2, green2, blue2, max, factor2, grey, r, g, b;
...
export default function w3colorToRgbString(c) {
  const color = w3color(c, null);
  if (color.valid) {
    if (color.opacity < 1) {
